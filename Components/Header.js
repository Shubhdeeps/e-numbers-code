...
const Header = ({number, setFilterItems, setSingleComponent, value}) => {
    ...
    useEffect(() => {
        ...
        const Items = number.filter(x => {
            return(x.Number.match(value))
        })
        if(Items.length === 1){
            setSingleComponent(Items)
            console.log(Items)
        }
        else{
            setSingleComponent('')
    ...
    return(
        <div id='appbar'>
            <h1> <span> E </span>NUMBERS</h1>
            <div id='inputBox' >
            <input ref={ref} onChange={(e) => handleChange(e)} placeholder="Search" />
